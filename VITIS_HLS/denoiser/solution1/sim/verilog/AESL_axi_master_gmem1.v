...
//------------------------Parameter----------------------
`define TV_IN_gmem1 "../tv/cdatafile/c.denoiser_top.autotvin_gmem1.dat"
`define TV_OUT_gmem1 "../tv/rtldatafile/rtl.denoiser_top.autotvout_gmem1.dat"
...
parameter gmem1_mem_depth = 32'd 784;
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem1_mem_depth) begin
                $display ("C:/Nam_3_HK2_2025_2026/Do_an_1/c_inter_to_hls/denoiser/solution1/sim/verilog/AESL_axi_master_gmem1.v: Write request address %d exceed AXI master gmem1 array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem1_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem1_mem_depth) begin
                $display ("C:/Nam_3_HK2_2025_2026/Do_an_1/c_inter_to_hls/denoiser/solution1/sim/verilog/AESL_axi_master_gmem1.v: Read request address %d exceed AXI master gmem1 array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem1_mem_depth); 
                $finish;
            end
...
initial begin : write_file_proc 
  ...
  while(1) begin 
      ...
      bin_data = gmem1_mem_depth;
      write_binary(fp,bin_data,64);
      for(i = 0; i < (gmem1_mem_depth - gmem1_mem_depth % factor); i = i + 1) begin 
          if (factor == 4) begin
              case(mem_page)
                  0 : tmp_data = gmem1_mem_0[i/factor];
                  1 : tmp_data = gmem1_mem_1[i/factor];
                  2 : tmp_data = gmem1_mem_2[i/factor];
          ...
          if (factor == 1) begin
              case(mem_page)
                  0 : mem_tmp = gmem1_mem_0[i];
                  1 : mem_tmp = gmem1_mem_1[i];
                  2 : mem_tmp = gmem1_mem_2[i];
                  default: $display("The page_num of write file is not valid!");
              endcase
          bin_data = mem_tmp;
          write_binary(fp,bin_data,16);
      ...
      if (factor == 4) begin
          case(mem_page)
              0 : tmp_data = gmem1_mem_0[gmem1_mem_depth/factor];
              1 : tmp_data = gmem1_mem_1[gmem1_mem_depth/factor];
              2 : tmp_data = gmem1_mem_2[gmem1_mem_depth/factor];
              default: $display("The page_num of write file is not valid!");
          endcase
          if ((gmem1_mem_depth - 1) % factor == 2) begin
          ...
          if ((gmem1_mem_depth - 1) % factor == 1) begin
              bin_data = tmp_data[7:0];
              write_binary(fp,bin_data,8);
              bin_data = tmp_data[15:8];
              write_binary(fp,bin_data,8);
          end
          if ((gmem1_mem_depth - 1) % factor == 0) begin
              bin_data = tmp_data;
              write_binary(fp,bin_data,8);
          end
      end
      if (factor == 2) begin
          if ((gmem1_mem_depth - 1) % factor == 0) begin
              case(mem_page)
                  0 : tmp_data = gmem1_mem_0[gmem1_mem_depth/factor][15:0];
                  1 : tmp_data = gmem1_mem_1[gmem1_mem_depth/factor][15:0];
                  2 : tmp_data = gmem1_mem_2[gmem1_mem_depth/factor][15:0];
                  default: $display("The page_num of write file is not valid!");
              endcase
              bin_data = tmp_data;
              write_binary(fp,bin_data,16);

...
typedef struct {
    ...
    XDenoiser_top_uio_map maps[ MAX_UIO_MAPS ];
} XDenoiser_top_uio_info;

/***************** Variable Definitions **************************************/
static XDenoiser_top_uio_info uio_info;
...
static int uio_info_read_name(XDenoiser_top_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XDenoiser_top_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XDenoiser_top_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XDenoiser_top_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XDenoiser_top_Initialize(XDenoiser_top *InstancePtr, const char* InstanceName) {
	XDenoiser_top_uio_info *InfoPtr = &uio_info;
